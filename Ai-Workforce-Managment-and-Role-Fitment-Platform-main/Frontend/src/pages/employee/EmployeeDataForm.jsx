...
import { addEmployee } from "@/servicess/api";

/**
 * EmployeeDataForm
 *
 * Simple manual entry form for managers to add a new employee record.
 */
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                productivity: form.productivity ? parseInt(form.productivity) : 0,
                utilization: form.utilization ? parseInt(form.utilization) : 0,
                fitmentScore: form.fitmentScore ? parseFloat(form.fitmentScore) : 0,
            };
            const res = await addEmployee(payload);
            if (res.data?.success) {
                toast({ title: "Employee added", description: "Record saved successfully." });
                navigate("/employees");
            } else {
                throw new Error(res.data?.error || "Unknown response");
            }
        } catch (err) {
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <Input id="salary" name="salary" type="number" value={form.salary} onChange={handleChange} />
                        </div>
                        <div>
                            <Label htmlFor="productivity">Productivity</Label>
                            <Input id="productivity" name="productivity" type="number" value={form.productivity} onChange={handleChange} />
                        </div>
                        <div>
                            <Label htmlFor="utilization">Utilization</Label>
                            <Input id="utilization" name="utilization" type="number" value={form.utilization} onChange={handleChange} />
                        </div>
                        <div>
                            <Label htmlFor="fitmentScore">Fitment Score</Label>
                            <Input id="fitmentScore" name="fitmentScore" type="number" value={form.fitmentScore} onChange={handleChange} />
                        </div>
                    </div>
