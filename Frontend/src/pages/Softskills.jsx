...
export default function Softskills() {
  ...
  const riskSignals = useMemo(() => {
    const highBurnout = employees.filter(e => e.scores.fatigue > 80);
    const skillGap = employees.filter(e => e.scores.fitment < 70);
    const lowLeadership = employees.filter(e => e.scores.aptitude < 65);
    const criticalAttrition = employees.filter(e => e.scores.fatigue > 85 && e.scores.utilization > 90);
  ...
  const opportunities = useMemo(() => {
    const promoReady = employees.filter(e => e.scores.fitment >= 90 && e.scores.fatigue < 50);
    const coachingReq = employees.filter(e => e.scores.skill < 75);
    const reassignment = employees.filter(e => e.scores.skill > 85 && e.scores.fitment < 75);
    const leadershipPipe = employees.filter(e => e.scores.aptitude > 85 && e.scores.fitment >= 80);
          ...
          <div className="py-8 space-y-8">
            ...
            <div className="pt-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {employees.sort((a, b) => b.scores.skill - a.scores.skill).slice(0, 4).map((e, idx) => (
