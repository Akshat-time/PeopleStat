...
const EmployeeDataForm = () => {
  ...
  if (!employees) return <div>Loading workforce data...</div>;
    ...
    // Find the current employee's data or use a fallback. Ensure it's never undefined.
    ...
    const [formData, setFormData] = useState({
        ...
        employeeName: currentEmployee?.name || user?.email?.split('@')[0] || "Employee",
        employeeId: currentEmployee?.employeeId || "EMP-001",
        department: currentEmployee?.department || "Finance",
        currentTeamName: "",
        designation: currentEmployee?.position || currentEmployee?.recommendedRole || "Senior Analyst",
        grade: "Grade 2",
        band: "D2",
        location: "Mumbai",
