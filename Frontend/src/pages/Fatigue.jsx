...
export default function Fatigue() {
  ...
  const fatigueMetrics = useMemo(() => {
    const avgFatigue = employees.reduce((sum, e) => sum + e.scores.fatigue, 0) / employees.length;
    return {
  ...
  const keyIndicators = useMemo(() => [
    {
      ...
      value: Math.round(employees.reduce((sum, e) => sum + e.scores.utilization, 0) / employees.length),
      change: 12,
      changeType: "up",
      icon: Target,
    ...
    },
    {
      ...
      value: Math.round(employees.reduce((sum, e) => sum + e.scores.productivity, 0) / employees.length),
      change: 15,
      changeType: "down",
      icon: Activity,
    ...
    },
    {
      ...
      value: Math.round(employees.reduce((sum, e) => sum + e.scores.fatigue, 0) / employees.length),
      change: 9,
      changeType: "up",
      icon: Heart,
  ...
  const wellbeingSignals = useMemo(() => [
    {
      ...
      count: employees.filter(e => e.scores.fatigue >= 75).length,
      employees: employees.filter(e => e.scores.fatigue >= 75),
      factor: "Extended high utilization (>95%)",
      color: "red",
    },
    {
      title: "Low Engagement",
      count: employees.filter(e => e.scores.productivity < 65).length,
      employees: employees.filter(e => e.scores.productivity < 65),
      factor: "Repetitive task cycles",
      color: "yellow",
    },
    {
      title: "High Stress Exposure",
      count: employees.filter(e => e.scores.fatigue >= 50 && e.scores.fatigue < 75).length,
      employees: employees.filter(e => e.scores.fatigue >= 50 && e.scores.fatigue < 75),
      factor: "Irregular working patterns",
      color: "orange",
    },
    {
      title: "Low Recovery Time",
      count: employees.filter(e => e.scores.utilization > 90).length,
      employees: employees.filter(e => e.scores.utilization > 90),
