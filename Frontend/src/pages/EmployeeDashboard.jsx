    ...
    return (
        <div className="space-y-10">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-4xl font-extrabold tracking-tight">
                        Welcome back, {employeeData?.name || user?.username || "Employee"}
                    </h1>
            ...
            <div className="grid md:grid-cols-3 gap-6">
                <MetricCard
                    title="Fitment Score"
                    value={(employeeData?.scores?.fitment || 0) + "%"}
                    icon={Target}
                ...
                <MetricCard
                    title="Utilization"
                    value={(employeeData?.scores?.utilization || 0) + "%"}
                    icon={Zap}
                ...
                <MetricCard
                    title="Fatigue Level"
                    value={(employeeData?.scores?.fatigue || 0) + "%"}
                    icon={AlertTriangle}
                    description="Burnout risk indicator"
                    color={(employeeData?.scores?.fatigue || 0) > 70 ? "text-destructive" : "text-green-600"}
                    onClick={() => go("/fatigue")}
                />
            </div>
                    ...
                    <CardContent className="space-y-6">
                        <div>
                            <div className="flex justify-between text-sm mb-2">
                                ...
                                <span className="text-muted-foreground">{employeeData?.scores?.productivity || 0}%</span>
                            </div>
                            <Progress value={employeeData?.scores?.productivity || 0} className="h-2" />
                        </div>

                        <div className="space-y-3">
                            <h4 className="text-sm font-semibold">Top Skills</h4>
                            <div className="flex flex-wrap gap-2">
                                {[...(employeeData?.skills?.hard || []), ...(employeeData?.skills?.soft || [])].slice(0, 5).map(skill => (
                        ...
                        <div className="pt-4 border-t">
                            ...
                            <p className="text-lg font-bold text-blue-600">{employeeData?.recommendedRole || employeeData?.position || "Senior Analyst"}</p>
                    ...
                    <CardContent className="space-y-4">
                        <div className="p-4 bg-white rounded-xl border border-indigo-100 shadow-sm">
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                                Your high {employeeData?.skills?.hard?.[0] || 'Domain'} proficiency puts you in the top 15% of candidates for {employeeData?.recommendedRole || employeeData?.position || "Senior Analyst"} positions.
                                Focus on {employeeData?.skills?.soft?.[0] || 'Leadership'} to fast-track your next promotion.
                            </p>
                        </div>
